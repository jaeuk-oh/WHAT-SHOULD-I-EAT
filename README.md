...
- **가상 냉장고** — 홈 화면 "내 재료" 섹션을 리스트 대신 냉장고 모양으로 볼 수 있는 토글. 실제 브랜드/모델이 아니라
  스타일화된 3가지 타입(`users/{uid}.fridgeType` — 미니/일반/양문형, 설정에서 변경) 중 하나로 렌더링 —
  미니는 좁은 외문형, 일반은 냉동실 칸이 있는 외문형, 양문형은 두 짝 문으로 구조 자체가 다르게 그려짐
  (초기 버전은 셋이 거의 똑같아 보인다는 피드백을 받고 다시 그림). 재료를 터치해서 여러 개 고르면
  "선택한 N개로 추천받기" 버튼이 뜨고, 그 재료들만으로 AI 추천을 새로 받아옴
- **냉장고 입장 트랜지션** — 로그인 직후 문을 열면(스와이프 또는 "들어가기" 버튼) 문이 열리고, 잠깐 멈췄다가
  카메라가 안쪽 빛을 향해 확 빨려 들어가듯 확대되며 화면이 밝게 번쩍인 뒤 그 자리에서 홈 화면이 드러남
...
- 가상 냉장고(홈 화면 "냉장고로 보기" 토글, 냉장고 기종 설정)는 선택한 재료로 `/recipes?prompt=...`를
  통해 추천을 새로 받아오는 흐름까지는 코드 리뷰로만 검증했다. 세 기종(미니/일반/양문형)의 구조가
  실제로 다르게 그려지는지는 로그인 없이 컴포넌트만 임시 페이지에 띄워 브라우저로 직접 확인했지만,
  실제 홈 화면 폭(카드 레이아웃)에서도 자연스러운지는 로그인 후 화면이라 확인 못했다.
...
- 냉장고 입장 트랜지션(문이 열린 뒤 안쪽 빛으로 확대되며 화면이 번쩍이는 연출)도 로그인 없이 컴포넌트만
  임시 페이지에 띄워 "들어가기" 버튼으로 문 열림 → 확대 → 종료까지 끊기거나 멈추지 않고 이어지는 것은
  확인했다. 다만 실제 드래그 스와이프 제스처(위 항목)와 마찬가지로 실기기 터치로는 확인하지 못했고,
  확대·번쩍임의 타이밍이 실제로 자연스럽게 느껴지는지는 사람이 봐야 알 수 있다.
