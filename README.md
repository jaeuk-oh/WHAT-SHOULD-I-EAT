...
- **AI 레시피 추천** — 임박 재료 우선, 자유 프롬프트/카테고리 필터/재추천, 대체 재료 안내.
  ...
  프롬프트에 "실존하는 요리만, 가진 재료를 최대한 활용" 규칙을 명시하고, 응답이 와도 서버에서 한 번 더
  missingIngredients 적은 순으로 재정렬해 모델이 지시를 놓쳐도 결과가 크게 어긋나지 않게 함
...
- **가상 냉장고** — 홈 화면 "내 재료" 섹션을 리스트 대신 냉장고 모양으로 볼 수 있는 토글. 실제 브랜드/모델이 아니라
  스타일화된 3가지 타입(`users/{uid}.fridgeType` — 미니/일반/양문형, 설정에서 변경) 중 하나로 렌더링. 재료를
  터치해서 여러 개 고르면 "선택한 N개로 추천받기" 버튼이 뜨고, 그 재료들만으로 AI 추천을 새로 받아옴
...
- 가상 냉장고(홈 화면 "냉장고로 보기" 토글, 냉장고 기종 설정)는 타입체크·빌드만 통과했고 로그인 후
  화면이라 브라우저로 직접 확인하지 못했다. 선택한 재료로 `/recipes?prompt=...`를 통해 추천을 새로
  받아오는 흐름도 코드 리뷰로만 검증했다.
- 레시피 추천 프롬프트에 "실존하는 요리만", "가진 재료 최대한 활용" 규칙을 추가하고 서버에서 결과를
  재정렬하는 로직도 넣었지만, 실제 NVIDIA NIM 응답 품질이 체감상 얼마나 나아졌는지는 API 키를 쓰는
  호출이라 이번 세션에서 확인하지 못했다. 코드는 타입체크·빌드만 통과한 상태다.
